...
% below).
%
% NP is the number of parameters in the function llfunc.
%
...
fitparams.maxFminIter = 1e10; %1e8                      % fminunc max iterations
fitparams.maxFminEval = 1e10; %1e8                      % fminunc max evaluations
fitparams.robust = 5; %3    							% restarts of each individual fit (at least 1)
fitparams.maxEmIter = 1e-3;                             % EM convergence criterion
fitparams.posdefify = 1;                                % force eigenvalues of nu to be positive
...
if nargin>4 & ~isempty(t{5}); maxit       = t{5}; else maxit = 2000;      end;
...
if nograd; 													% assume gradients are supplied
    fminopt=optimoptions('fminunc','display','off',...
        'TolFun',fitparams.tol, 'maxFunEvals', fitparams.maxFminEval, 'MaxIterations', fitparams.maxFminIter);
else
    fminopt=optimoptions('fminunc','display','off','GradObj','on','algorithm','trust-region', ...
        'TolFun',fitparams.tol, 'maxFunEvals', fitparams.maxFminEval, 'MaxIterations', fitparams.maxFminIter);
    if docheckgrad; 										% if they are, then can check them.
        fminopt=optimoptions('fminunc','display','off','GradObj','on','DerivativeCheck','on',...
            'TolFun',fitparams.tol, 'maxFunEvals', fitparams.maxFminEval, 'MaxIterations', fitparams.maxFminIter);
    end
end
...
while 1;emit=emit+1; t0=tic;
    ...
    parfor sj=sjind_pf; tt0=tic;
        ...
        while 1 												% deal with poor convergence failures and poor initial values
            ...
            try
                [est(:,nfc),fval(nfc),ex(nfc),~,~,hess(:,:,nfc)] = fminunc(@(x)fstr(x,D(sk),musj(:,sk),nui,doprior,llopt),init,fminopt);
            catch me
                e=real(sqrtm(nu))*randn(Np,1);
                ex(nfc)=0;
            end
            %             if any(ex(nfc)==[1:3]); break;end
            if any(ex(nfc)>0); break;end
            if ~mod(nfc,50); fprintf('\nsj %i not converging (it %i; error: %s).',sk,nfc,me.message); end
        end
        
        [~,best] = min(fval);							% take fit that led to best function value among those that converged
    ...
    if emit> 1; 							% only update priors after first MAP iteration
        while 1								% iterate over prior mean and covariance updates until convergence
            ...
            % prior mean update - note prior mean is different for each
            % subject if GLM is included
            alpha_old = alpha;
            ah = zeros(Npall,1);
            vh = zeros(Npall);
            for  sj=1:Nsj
            ...
            [eigV,eigD] = eig(nunew);
            d = diag(eigD);
            if any(d < 1e-4); fprintf(' nufull not positive definite. posdefifying. ');
                d(d<1e-4) = 1e-4;
            end
            nu = eigV*diag(d)*eigV';

            %             if det(nunew)<=0; fprintf(' nufull not positive definite. ');
            %                 if fitparams.posdefify == 1
            %                     fprintf(' posdefifying. ');
    ...
    if emit>1;if abs(sum(PL)-PLold) < fitparams.maxEmIter; nextbreak=1;stats.ex=1; fprintf(' ...converged ');end;end
    ...
    % save intermediate fit
    if ~isempty(savestr); eval(['save ' savestr ' E V alpha stats emit musj nui fitparams']);end
...
if ~isempty(savestr);
    ...
    eval(['save ' savestr ' D E V alpha stats emit musj nui bf fitparams']);
end
